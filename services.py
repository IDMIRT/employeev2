    ...
    """
     try:
        ...
        client.containers.run(
            ...
            ports={'5432/tcp': 5432},
            volumes={
                str(database_dir): {'bind': '/var/lib/postgresql/', 'mode': 'rw'}
            },
            detach=True)
        ...
        for attempt in range(60): #на range(10) не успевает запустится и проверка падает
            import psycopg2
            try:
                dsn = f"postgresql+psycopg2://{user}:{password}@localhost:5432/employee" #для SQLAlchemy
                #для psycopg2 используем прямое подключение               
                conn = psycopg2.connect(f"postgresql://{user}:{password}@localhost:5432/employee")
                conn.close()                
